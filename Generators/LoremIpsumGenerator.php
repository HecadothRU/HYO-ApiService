...
class LoremIpsum
{
...
}      function lorem_ipsum_generator($type, $amount) {
        $data = [];
        $Lipsum = new LoremIpsum();
        switch($type)
        {
          case 'paragraphs':
          $data['result'] = $Lipsum->paragraphs($amount);
          break;
          case 'sentences':
          $data['result'] = $Lipsum->sentences($amount);
          break;
          case 'words':
          $data['result'] = $Lipsum->words($amount);
          break;
        }
      $values = [
        'amount' => $amount ?? 1,
    ...
    if (isset($_GET['type']))
    {
      if (isset($_GET['amount'])){
        $response = lorem_ipsum_generator($_GET['type'], $_GET['amount']);
        die($response);
      } else {
        die("Missing GET Parameter | Set and Retry | [amount(int)]");
      }
    } else {
      die("Missing GET Parameter | Set and Retry | [type] ~> [words|sentences|paragraphs]");
    }
